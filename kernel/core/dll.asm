...
proc attach_int_handler stdcall, irq:dword, handler:dword, access_rights:dword
        ...
        test    ebx, ebx
        jz      .err
        cmp     ebx, 15 ; we only have 16 IRQ's
        ja      .err
        mov     eax, [handler]
        test    eax, eax
        jz      .err
        cmp     [irq_owner + ebx * 4], 0
        je      @f

        mov     ecx, [irq_rights + ebx * 4] ; Rights : 0 - full access, 1 - read only, 2 - forbidden
        test    ecx, ecx
        jnz     .err
    ...
    @@: mov     [irq_tab + ebx * 4], eax
        ...
        mov     [irq_rights + ebx * 4], eax

        mov     [irq_owner + ebx * 4], PID_KERNEL ; all handlers belong to a kernel
...
init_dlls_in_thread:
        ...
        ; DLL is per-process object, so app_data_t.dlls_list_ptr must be
        ; kept in sync for all threads of one process.
        ; out: eax = app_data_t.dlls_list_ptr if all is OK,
...
dereference_dll:
        ; in: eax = number of references to delete, esi -> dll_descriptor_t struc
...
destroy_all_hdlls:
        ; ecx -> app_data_t for slot, esi = dlls_list_ptr
